...
int main()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_DEPTH_COMPONENT, SHADOW_WIDTH, SHADOW_HEIGHT, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL); // tell it to make a depth map texture
    }
    ...
    glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, depthMap, 0); // apply depthmap to frame buffer
    glDrawBuffer(GL_NONE);                                                  // These are here so that openGL knows that we dont want color
    glReadBuffer(GL_NONE);
    glBindFramebuffer(GL_FRAMEBUFFER, 0);

    //-----------Create and compile shaders-------------------------------
    Shader depthShader("../resources/shaders/omnidirectional_shadow/shadowVertex.vs", "../resources/shaders/omnidirectional_shadow/shadowFragment.fs", "../resources/shaders/omnidirectional_shadow/shadowGeometry.gs");
    Shader sceneShader("../resources/shaders/omnidirectional_shadow/vertex.vs", "../resources/shaders/omnidirectional_shadow/fragment.fs");
    //---------------------Texture Loading--------------------------------

    unsigned int floorDiffuse = loadTexture2D("../resources/textures/floor/floor_Diffuse.jpg", false);

    glUniform1f(glGetUniformLocation(sceneShader.ID, "diffuseTex"), 0);
    glUniform1i(glGetUniformLocation(sceneShader.ID, "depthMap"), 1);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 lightProjectionMat = glm::perspective(glm::radians(90.0f), (float)SHADOW_WIDTH / (float)SHADOW_HEIGHT, nearPlane, farPlane);

        std::vector<glm::mat4> shadowTransforms;
        shadowTransforms.push_back(lightProjectionMat * glm::lookAt(lightPos, lightPos + glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f)));
        shadowTransforms.push_back(lightProjectionMat * glm::lookAt(lightPos, lightPos + glm::vec3(-1.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f)));
        shadowTransforms.push_back(lightProjectionMat * glm::lookAt(lightPos, lightPos + glm::vec3(0.0f, 1.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f)));
        shadowTransforms.push_back(lightProjectionMat * glm::lookAt(lightPos, lightPos + glm::vec3(0.0f, -1.0f, 0.0f), glm::vec3(0.0f, 0.0f, -1.0f)));
        shadowTransforms.push_back(lightProjectionMat * glm::lookAt(lightPos, lightPos + glm::vec3(0.0f, 0.0f, 1.0f), glm::vec3(0.0f, -1.0f, 0.0f)));
        shadowTransforms.push_back(lightProjectionMat * glm::lookAt(lightPos, lightPos + glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, -1.0f, 0.0f)));

        glm::mat4 projectionMat = glm::perspective(glm::radians(camera.Zoom), (float)SCREEN_WIDTH / (float)SCREEN_HEIGHT, 0.1f, 100.0f);
        ...
        for (unsigned int i = 0; i < 6; ++i)
            glUniformMatrix4fv(glGetUniformLocation(depthShader.ID, ("shadowMatrices[" + to_string(i) + "]").c_str()), 1, GL_FALSE, glm::value_ptr(shadowTransforms[i]));
        glUniform1f(glGetUniformLocation(depthShader.ID, "far_plane"), farPlane);
        glUniform3f(glGetUniformLocation(depthShader.ID, "lightPos"), lightPos.x, lightPos.y, lightPos.z);
        ...
        sceneShader.use();
        glUniformMatrix4fv(glGetUniformLocation(sceneShader.ID, "projectionMat"), 1, GL_FALSE, glm::value_ptr(projectionMat));
        glUniformMatrix4fv(glGetUniformLocation(sceneShader.ID, "viewMat"), 1, GL_FALSE, glm::value_ptr(viewMat));
        glUniform3f(glGetUniformLocation(sceneShader.ID, "lightPos"), lightPos.x, lightPos.y, lightPos.z);
        glUniform3f(glGetUniformLocation(sceneShader.ID, "viewPos"), camera.Position.x, camera.Position.y, camera.Position.z);
        glUniform1f(glGetUniformLocation(sceneShader.ID, "farPlane"), farPlane);
...
void renderScene(const Shader &shader)
{
    ...
    renderCube();
    glUniform1i(glGetUniformLocation(shader.ID, "reverseNormals"), 0);
    ...
    modelMat = glm::mat4(1.0f);
    modelMat = glm::translate(modelMat, glm::vec3(-1.5f, 1.0f, 0.0f));
    modelMat = glm::scale(modelMat, glm::vec3(0.5f));
    glUniformMatrix4fv(glGetUniformLocation(shader.ID, "modelMat"), 1, GL_FALSE, glm::value_ptr(modelMat));
    renderCube();

    modelMat = glm::mat4(1.0f);
    modelMat = glm::translate(modelMat, glm::vec3(-1.5f, 2.0f, 0.0f));
