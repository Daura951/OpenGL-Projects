...
int main()
{
    ...
    Shader shader("../resources/shaders/shadow_map/vertex.vs", "../resources/shaders/shadow_map/fragment.fs");
    //---------------------Texture Loading--------------------------------
    ...
    shader.use();
    glUniform1f(glGetUniformLocation(shader.ID, "diffuseTex"), 0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glUniformMatrix4fv(glGetUniformLocation(shader.ID, "lightSpaceMat"), 1, GL_FALSE, glm::value_ptr(lightSpaceMat));
        ...
        glBindTexture(GL_TEXTURE_2D, depthMap);
        glCullFace(GL_FRONT); // help eliminate peter panning on objects that have insides
        renderScene(shader);
        glCullFace(GL_BACK); // turn back to normal! :)

        // quad in order to see maps!
        quadShader.use();
