...
vec2 parallaxMapping(vec2 texCoord, vec3 viewDir)
{
    ...
    while(currentDepth < curDepthMapVal) //steep parallax time! go until our map read exceeds the depth on view direction
    {
        curTexCoord -= deltaTexCoord; //go down scaled viewDir by step
        curDepthMapVal = texture(displacementMap, curTexCoord).r; //resample from map
        currentDepth+=layerDepth; //go deeper in depth on view direction
    }
